...
const MalfunctionDetails = ({ malfunction, onClose }) => {
  ...
  const translateStatus = (status) => {
    const translations = {
      ...
      'pending_maintenance': 'En attente de maintenance',
      'good': 'Bon état'
    };
              ...
              <div className="flex-grow overflow-y-auto p-6">
                <div className="flex w-full flex-col md:flex-row h-full justify-between items-start p-4">
                  <div className="w-full md:w-1/2 pr-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <DetailItem label="Date de déclaration" value={new Date(malfunction.declared_date).toLocaleDateString()} />
                      <DetailItem label="Heure de déclaration" value={malfunction.declared_time} />
                    </div>
                  </div>
                ...
                <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    ...
                    <DetailItem label="Dernière maintenance" value={new Date(malfunction.last_maintenance_date).toLocaleDateString()} />
                    <DetailItem label="Prochaine inspection" value={new Date(malfunction.inspection_due_date).toLocaleDateString()} />
                  </div>
                    ...
                    <div className="flex items-center">
                      ...
                      <span 
                        className={`px-3 py-1 rounded-full text-sm ${
                          malfunction.is_active 
                            ? 'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100' 
                            : 'bg-blue-100 text-blue-800 dark:bg-blue-800 dark:text-blue-100'
                        }`}
                      >
                        {malfunction.is_active ? 'Actif' : 'hors service'}
                      </span>
                    </div>
                  </div>
                </div>
