// src/pages/announcements/UpdateAnnouncementModal.jsx
...
const UpdateAnnouncementModal = ({
  ...
  onClose,
  announcement,
  onUpdateSuccess, // preferred callback from parent
  onSubmit // kept for backward compatibility if parent still uses onSubmit
}) => {
  ...
  useEffect(() => {
    if (announcement) {
      setFormData({
        ...
        cover: announcement.cover || null,
        coverFile: null,
        pdfUrl: announcement.pdfUrl || null,
        pdfFile: null,
        dbId: announcement.dbId || null
      });
      setErrors({});
      setMessage('');
      setMsgSuccess(false);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.dbId) {
      setMessage("Identifiant de l'annonce manquant");
      setMsgSuccess(false);
      setShowMessage(true);
    ...
    try {
      ...
      // keep keys consistent with your create modal (you used startDate / endDate there),
      // backend appears to accept them in your setup — change to start_date/end_date if needed.
      payload.append('startDate', formData.startDate);
      payload.append('endDate', formData.endDate);
      payload.append('created_by', formData.createdBy || 'Admin');

      if (formData.coverFile) {
        payload.append('coverFile', formData.coverFile);
      } else if (formData.cover === null) {
        // if cover was removed, send an empty marker (depends on backend behavior)
      ...
      if (formData.pdfFile) {
        ...
        // marker to remove existing pdf if user cleared it
        payload.append('pdf', '');
      }

      const response = await VeYooAxios.put(
        `/advertisements/advertisement-update/${formData.dbId}/`,
        payload,
        {
      ...
      // Build updated announcement object to send back to parent (same shape parent expects)
      const d = response.data;
      const updatedAnnouncement = {
        dbId: d.id ?? formData.dbId,
        id: d.id ?? formData.dbId,
        title: d.title ?? formData.title,
        content: d.content ?? formData.content,
        priority: d.priority ?? formData.priority,
        startDate: d.start_date ?? d.startDate ?? formData.startDate,
        endDate: d.end_date ?? d.endDate ?? formData.endDate,
        createdBy: d.created_by ?? formData.createdBy,
        updatedBy: d.updated_by ?? undefined,
        cover: d.cover_base64 ? `data:image/jpeg;base64,${d.cover_base64}` : formData.cover,
        pdfUrl: d.pdf_base64 ? `data:application/pdf;base64,${d.pdf_base64}` : formData.pdfUrl,
        targetRoles: ['All'],
        // derive status locally
        status: (() => {
          try {
            const now = new Date();
            const s = new Date(d.start_date ?? formData.startDate);
            const e = new Date(d.end_date ?? formData.endDate);
      ...
      // Notify parent (use either onUpdateSuccess or legacy onSubmit)
      try {
        if (typeof onUpdateSuccess === 'function') onUpdateSuccess(updatedAnnouncement);
        if (typeof onSubmit === 'function') onSubmit(updatedAnnouncement);
      } catch (cbErr) {
        // ignore callback errors but show message
        console.warn('Update callback error', cbErr);
      }
      ...
      setMessage(err.response?.data?.message || 'Erreur lors de la mise à jour de l\'annonce');
      setMsgSuccess(false);
      setShowMessage(true);
  ...
  return (
    <>
      ...
      {isSubmitting && <Loading loading_txt={'Mise à jour en cours...'} />}
              ...
              <div>
                ...
                <div className="flex items-center space-x-4">
                  ...
                  {formData.cover && (
                    <button
                      type="button"
                      onClick={() => setFormData(prev => ({ ...prev, cover: null, coverFile: null }))}
              ...
              <div>
                ...
                <div className="flex items-center space-x-4">
                  ...
                  {formData.pdfUrl && (
                    <button
                      type="button"
                      onClick={() => setFormData(prev => ({ ...prev, pdfUrl: null, pdfFile: null }))}
                      className="text-sm text-red-600 hover:text-red-800"
                    >
                      Supprimer
                    </button>
              ...
              <div className="flex justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
                ...
                <button
                  ...
                  className={`flex items-center space-x-2 px-4 py-2 text-sm font-medium text-white rounded-lg transition-colors ${
                    isSubmitting ? 'bg-gray-500 cursor-not-allowed' : 'bg-blue-600 dark:bg-blue-500 hover:bg-blue-700 dark:hover:bg-blue-600'
                  }`}
                >
                  {isSubmitting ? (
