...
const EmailCodeConfirmationModal = ({
  isOpen,
  onClose,
  onCodeConfirmed,
  email
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      handleSendCode(); // Automatically send code when modal opens
    }
  ...
  const handleSendCode = async () => {
    ...
    try {
      const response = await VeYooAxios.post('/auth/send-email-confirmation-code/', { email });
      setMessage(response.data.message || 'Code de confirmation envoyé à votre email.');
      setIsSuccess(true);
      setShowMessage(true);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await VeYooAxios.post('/auth/confirm-email/', { code: fullCode });
      ...
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white dark:bg-gray-800 shadow-xl rounded-lg">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-medium text-gray-900 dark:text-white">Confirmation du code email</h3>
          ...
          <div className="mb-6 text-center">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">Nous avons envoyé un code de confirmation à 6 chiffres à</p>
...
export default EmailCodeConfirmationModal;